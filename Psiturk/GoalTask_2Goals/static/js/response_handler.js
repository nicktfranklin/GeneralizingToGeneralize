    ...
    return function (event) {
        ...
        for (var agent in this.state) {
            ...
            if (this.mdp.inGoal(nextState[agent]['location'], agent)) {
                ...
                // if agent is in goal state, celebrate.
                var celebrateGoal = (function (painter, location, agent, points, total_points, goal_display_label) {
                    return function () {
                        if (goal_value > 0) {
                        ...
                        var text_display = 'Goal: <span style="font-weight: bold"><span style="font-size:150%">' +
                            ...
                            goal_display_label +'</span></span></span><br>Total Points won: <span style="color:' +
                            painter.AGENT_COLORS['agent1'] +'"><span style="font-size:150%">' +
                            '<span style="font-weight: bold">' + String(total_points) + '</span></span></span><br> ' +
                            '<span style="font-style: italic"><span style="color: #707070">Press enter to continue' +
                            '</span></span>';

                        $('#trial_text').html(text_display);
                    }
                })(this.painter, nextState[agent]['location'], agent, goal_value, this.total_points, goal_display_label);
